...
void EEPROMSetup(void)
{
  ...
  if (!EEPROM.begin(EEPROM_SIZE)) {DebugPrintln("EEPROM setup problem", DBG_ERROR);}
  else {
...
//      Serial.print(i); Serial.print(" "); Serial.println(EEPROM.readByte(EEPROMBaseAdd+i));
      ...
      if (false){
        ...
        if(EEPROMLoad.LoadBuffer[i] <= 0X0F) {DebugPrint("0");}
        DebugPrint(String(EEPROMLoad.LoadBuffer[i], HEX)); DebugPrint(" ");
        count = count + 1;
        if (count > 15) {
          DebugPrintln("");
          count = 0;
...
// calculate checksum
  ...
  for (int i = 0; i < EEPROM_SIZE - 1; i++) {calculatedChecksum = calculatedChecksum + EEPROMLoad.LoadBuffer[i];}
